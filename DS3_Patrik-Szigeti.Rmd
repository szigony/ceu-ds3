...
set.seed(1234)
rf_model_mtry2 <- train(log_salary ~ .,
                        method = "ranger",
                        data = hitters_data,
...
set.seed(1234)
rf_model_mtry10 <- train(log_salary ~ .,
                        method = "ranger",
                        data = hitters_data,
...
The most important variables seem to be `CHits` (# of hits), `CAtBat` (# of times at bat), `CWalks` (# of walks), `CRuns` (# of runs) and `CRBI` (# of runs batted), all of which are related to overall career statistics. These variables are relatively close to each other in terms of importance.
...
`CAtBat` and `CHits` are still on the top of the list alongside with `CRuns`, but there's a sudden drop afterwards, and even these three are distributed more spaciously. The top 5 is the same as it was for `mtry = 2`, but steps are considerably larger.
