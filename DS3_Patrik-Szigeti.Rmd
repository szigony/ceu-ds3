...
The validation set performance seems to be best for the random forest model, followed closely by the deep learning model. GBM is relatively close to these two in terms of AUC, but GLM is significantly worse.
...
The correlation is highest between the random forest model and GBM based on the predictions on the validation set (0.88). GLM and GBM seem to mostly contradict each other, while the deep learning model's predictions correlate with each model about the same amount.
...
The ensemble model with `gbm` meta-learner still underperforms compared to the simple random forest model, the `deeplearning` meta-learner, however, helped improve performance. It shows more than 1% improvement compared to the base deep learning model, and performs better on the validation dataset than the random forest did.
